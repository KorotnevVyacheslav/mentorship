...
x = np.arange(0, np.pi/2, 0.000001)
plt.figure(figsize=(15, 10))
plt.plot(x,1 - TTM( np.sin(x) * np.sqrt(e1) , np.cos(x) * np.sqrt(e1) , e1 , e2 , a) - RTM( np.sin(x) * np.sqrt(e1) , np.cos(x) * np.sqrt(e1) , e1 , e2 , a),linewidth=2,alpha=0.8,color='black')
plt.plot(x, 1 - TTE( np.sin(x) * np.sqrt(e1) , np.cos(x) * np.sqrt(e1) , e1 , e2 , a) - RTE( np.sin(x) * np.sqrt(e1) , np.cos(x) * np.sqrt(e1) , e1 , e2 , a),linewidth=2,alpha=0.8,color='red')
plt.xlabel("$Угол$ $падения$ $theta$", size = 15)
plt.ylabel("$Амплитуда$ $отражения$ $A_TE$ $и$ $A_TM$",size = 15)
plt.title(r'$ATM$ $ATE$',size = 25)
plt.grid( True, 'major','x' )
plt.grid( True, 'major','y' )
plt.show()
"""
