...
def r_TM(phi, e1, e2, epsilon, mu, epsilon_2, mu_2, epsilon_3, mu_3):
    ...
    n_minus = n_epsilon_minus(
        phi, e1, e2, epsilon, mu, epsilon_2, mu_2, epsilon_3, mu_3
    )
    k1z = np.sqrt(0j + 1 - np.sin(phi) ** 2)
    # kz = np.sqrt(0j + epsilon * mu - np.sin(phi) ** 2)
    kz = np.sqrt(0j + epsilon * mu)
    r = (kz * n_plus - k1z * epsilon * n_minus)
    # / (kz * n_plus + k1z * epsilon * n_minus)
    return r
...
def r_TE(phi, e1, e2, epsilon, mu, epsilon_2, mu_2, epsilon_3, mu_3):
    ...
    # kz = np.sqrt(0j + epsilon * mu - np.sin(phi) ** 2)
    kz = np.sqrt(0j + epsilon * mu)
    r = (kz * n_plus - k1z * mu * n_minus)
    #/ (kz * n_plus + k1z * mu * n_minus)
    return r
...
def solutions(phi, a, b, epsilon_2, mu_2, epsilon_3, mu_3):
    ...
    def equation_coefficients(a1, b1, c1, a2, b2, c2):
        ...
        a2, b2, c2 - TE mode
        """
        tmp1 = 1 - b1 * b2 - a2 / 2 * (a1 - b1 * a2)
        tmp2 = c1 - b1 * c2
        tmp3 = (a1 - b1 * a2) ** 2
        A = tmp1 ** 2 - (a2 * a2 - 4 * b2) * tmp3 / 4
        B = 2 * tmp1 * tmp2 + c2 * tmp3
        C = tmp2 ** 2
        return [A, B, C]
    ...
    iter = 0
    for e in epsilons:
        mus2[iter] = e*(b2 / a2 - 1 / a1) + c2 / a2 - c1 / a1
        mus2[iter] = mus2[iter] / (b1 / a1 - 1 / a2)
        iter += 1
